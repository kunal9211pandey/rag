...
# Get exact torch version for correct PyG install
torch_ver = torch.__version__.split('+')[0]   # e.g. "2.0.0"
cuda_ver  = torch.version.cuda.replace('.','') # e.g. "117"

print(f"PyTorch: {torch.__version__}")
print(f"CUDA:    {torch.version.cuda}")
print(f"Installing torch-geometric for torch={torch_ver}+cu{cuda_ver}...")

# Install torch-geometric (works on Kaggle P100)
subprocess.run(['pip', 'install', 'torch-geometric', '-q'], check=True)

# Install scatter/sparse with correct URL
whl_url = f"https://data.pyg.org/whl/torch-{torch_ver}+cu{cuda_ver}.html"
subprocess.run([
...
def build_graph(points, p_field, U_field, inlet,
    ...
    p_std  = max(float(p_field.std()), 1e-8)
    U_mag  = max(float(np.linalg.norm(U_field, axis=1).max()), 1e-8)
